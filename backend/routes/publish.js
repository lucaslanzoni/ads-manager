...
router.post('/', async (req, res) => {
  const { sessionId, images, imageData = {}, brief, brandName, network, dailyBudget, startDate, endDate, pageId, destinationUrl, captions = {} } = req.body;
  ...
  try {
    const result = await publishCampaign({ sessionId, images, imageData, brief, brandName, network, dailyBudget, startDate, endDate, pageId, destinationUrl, captions });
    res.json(result);
  } catch (err) {
    console.error(err);
